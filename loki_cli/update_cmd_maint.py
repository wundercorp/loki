"""Post-update maintenance for ``loki update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary, stale-module purge.

Split out of ``update_cmd.py``, which re-imports every name so ``loki_cli.update_cmd.<name>``
...
from loki_constants import venv_python_path

from loki_cli.update_cmd_common import _best_effort

# Log-record parity with the origin module.
logger = logging.getLogger("loki_cli.update_cmd")


_UPDATE_RUNTIME_RELOAD_MODULES = "loki_constants", "tools.environments.local", "tools.lazy_deps"
...
_STALE_PURGE_PREFIXES = "loki_cli", "gateway", "tools", "tui_gateway", "agent"
...
_STALE_PURGE_PROTECTED = frozenset({"loki_cli", "loki_cli.main", "loki_cli.loki_logging"})
...
_STALE_PURGE_PROTECTED_PREFIX = "loki_cli.update_"
...
def _purge_stale_loki_modules() -> None:
    """Evict every cached Loki module after the checkout changed in-place. Never raises.
    ...
    """
    from loki_cli.update_cmd import _m
    with _best_effort('Could not purge stale Loki modules: %s'):
        importlib.invalidate_caches()
        ...
        if purged:
            logger.debug("Purged %d stale Loki module(s) after checkout update", len(purged))
...
def _print_fts_optimize_available_notice() -> None:
    ...
    try:
        ...
        from loki_state import SessionDB
    except Exception:
        return
    db_path = get_loki_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `loki sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    loki sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  loki sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: loki curator status)")
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from loki_cli._subprocess_compat import
    ...
    """
    _reload_modules(
        ("loki_cli._subprocess_compat", "loki_cli.dashboard_procs"),
        modules=sys.modules,
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from loki_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
    ...
    print("    loki dashboard --port <port>")
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from loki_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("LOKI_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== loki-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from loki_cli.update_cmd import _m
    from loki_constants import project_venv_dir
    from loki_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Loki runtime."""
    from loki_cli.update_cmd import _post_update_sqlite_runtime_status
    ...
    print("  Rebuild the Loki venv with a uv-managed Python, restart Loki, then verify with `loki doctor`.")
    return False
...
def _print_update_summary(
    *, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None, web_build_ok: bool = True
    ...
    See #88251.
    """
    from loki_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not web_build_ok or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `loki "
                "update` again; if SQLite is unchanged, rebuild the Loki venv "
                "with a uv-managed Python, restart Loki, then verify with "
                "`loki doctor`."
            )
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from loki_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from loki_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (loki gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Loki) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from loki_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: loki skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v loki'`` resolves."""
    from loki_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/loki).
    fhs_link = Path("/usr/local/bin/loki")
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `loki gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
    ...
    path_comment = "# Loki Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)"
    wrote_any = False
...
def _ensure_short_launcher() -> None:
    """Self-heal ``loki`` next to the canonical ``loki`` launcher on POSIX.

    Fresh installs create the pair in ``scripts/install.sh``. Updates can land on an older install,
    so create the short alias lazily without ever overwriting an existing user command. Windows is
    handled by the managed launcher repair path, which stages the ``loki`` console-script executable.
    """
    from loki_cli.update_cmd import _m
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            if short.exists() or short.is_symlink():
                continue
            try:
                short.symlink_to("loki")
            except OSError:
                shim = (
                    "#!/usr/bin/env bash\n"
                    "# Loki Agent — short launcher (written by `loki update`).\n"
                    f'exec "{canonical}" "$@"\n'
                )
                short.write_text(shim, encoding="utf-8")
                short.chmod(short.stat().st_mode | 0o755)
        except OSError:
            continue
        print(f"  ✓ Installed loki launcher → {short}")


def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``loki-acp`` launcher next to ``loki`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``loki acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$LokiHome\bin``, never
    ...
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        loki_cmd = bin_dir / "loki"
        acp_cmd = bin_dir / "loki-acp"
        try:
            if not (loki_cmd.is_file() or loki_cmd.is_symlink()):
                continue
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Loki Agent — ACP launcher (written by `loki update`).\n"
            ...
            continue
        print(f"  ✓ Installed loki-acp launcher → {acp_cmd}")
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    from loki_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
    from loki_cli.config import get_loki_home
    _src_path = get_loki_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_loki_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from loki_cli.update_cmd import _record_update_step
    from loki_cli.backup import create_quick_snapshot
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from loki_cli.backup import create_pre_update_snapshots_all_profiles
        ...
        if _sibling_snaps:
            ...
            import loki_cli.update_cmd_config as _cfg
...
def _run_full_backup() -> None:
    """Zip LOKI_HOME under ``backups/`` (restorable via ``loki import``). Never raises."""
    try:
        from loki_cli.backup import create_pre_update_backup
    except Exception as exc:
    ...
    from loki_cli.sizefmt import format_bytes
    # display_loki_home so the user sees ~/.loki/...
    try:
        from loki_constants import get_loki_home, display_loki_home
        display_path = f"{display_loki_home()}/{out_path.relative_to(get_loki_home())}"
    except Exception:
        display_path = str(out_path)

    print(f"  Saved:    {display_path} ({format_bytes(size_bytes)}, {elapsed:.1f}s)")
    print(f"  Restore:  loki import {out_path}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of LOKI_HOME under ``backups/`` (``loki import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in loki_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # LOKI_HOME, so sync_skills()'s module-level LOKI_HOME cache can't skew it.
    with suppress(Exception):
        from loki_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from loki_cli.profiles import backfill_profile_envs
...
def _refresh_cua_driver_after_update() -> None:
    ...
    if (
        ...
        from loki_cli.tools_config import install_cua_driver
        print()
...
def _print_plugin_compat_notice() -> None:
    ...
    from loki_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    """Best-effort notices (FTS optimize, curator) and self-heals (FHS PATH, short/ACP launchers,
    Windows bin launchers, cua-driver refresh) that run after the summary."""
    from loki_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from loki_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('loki short-launcher self-heal failed: %s', _ensure_short_launcher),
        ('loki-acp launcher self-heal failed: %s', _ensure_acp_launcher),
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from loki_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if Loki re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.wundercorp.lokiagent` (repeat "
    ...
    try:
        # See #95596.
        from loki_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
    ...
    with _best_effort('Model catalog seed during update failed: %s'):
        from loki_cli.model_catalog import seed_cache_from_checkout
