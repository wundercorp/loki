...
def _record_bytecode_fingerprint() -> None:
    ...
    from loki_cli.main import PROJECT_ROOT, _read_git_revision_fingerprint
    try:
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    Update-time clears can't close the stale-bytecode class: ``loki update`` runs
    ...
    pull inside ``loki update``, a manual ``git pull``, a ZIP update, a file-sync restore) while
    ...
    """
    from loki_cli.main import PROJECT_ROOT, _clear_bytecode_cache, _read_git_revision_fingerprint
    try:
        fingerprint = _read_git_revision_fingerprint(PROJECT_ROOT)
        if not fingerprint:
...
def _web_dist_dir(web_dir: Path) -> Path:
    """Vite outputs to ``loki_cli/web_dist/`` (vite.config.ts outDir), NOT ``web/dist/``."""
    return _web_project_root(web_dir) / "loki_cli" / "web_dist"
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    Content hash, NOT mtime: ``git checkout`` / ``loki update`` rewrite source
...
def _web_ui_stamp_path() -> Path:
    """Path of the web UI build stamp under $LOKI_HOME."""
    from loki_constants import get_loki_home
    return get_loki_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    half-state with the ``loki`` launcher present but ``loki_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    """``PYTHON=`` env for node-gyp on NixOS (bare PATH lookup fails outside nix-shell): the loki
...
def _run_npm_install_deterministic(
    ...
    from loki_cli.npm_engine import maybe_repair_npm_engine
    ...
    run_env["PATH"] = with_loki_node_path(run_env)["PATH"]
...
def _report_web_build_failure(step: str, result: subprocess.CompletedProcess, *, fatal: bool) -> bool:
    ...
    _console_print(f"  {'✗' if fatal else '⚠'} Web UI {step} failed" + ("" if fatal else " (loki web will not be available)"))
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False, require_fresh: bool = False) -> bool:
    ...
    soft warning (used by ``loki web``). Returns True when the build succeeded
    or was skipped (no package.json / up to date / stale dist served as fallback).
    """
    from loki_cli.main_install_repair import _resolve_node_runtime_npm
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_loki_node_path())
