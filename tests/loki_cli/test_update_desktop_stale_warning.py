...
#88251: ``loki update`` treated a failed desktop pack as non-fatal, printed
...
import loki_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_constants.with_loki_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "loki_constants.display_loki_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_loki_home", lambda: tmp_path)
